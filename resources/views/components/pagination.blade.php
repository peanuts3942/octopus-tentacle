...
<nav class="pagination" role="navigation" aria-label="Pagination">
    ...
    <div class="pagination-buttons">
        ...
        @else
            <a href="{{ $paginator->currentPage() == 2 ? route('home') : route('home.page', $paginator->currentPage() - 1) }}" class="pagination-button" rel="prev">
                <svg class="pagination-icon" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <polyline points="15 18 9 12 15 6"></polyline>
                </svg>
            </a>
        ...
        <div class="pagination-numbers">
            ...
            {{-- Première page --}}
            @if ($start > 1)
                <a href="{{ route('home') }}" class="pagination-number-button">1</a>
                @if ($start > 2)
            ...
            @for ($page = $start; $page <= $end; $page++)
                ...
                @else
                    <a href="{{ $page == 1 ? route('home') : route('home.page', $page) }}" class="pagination-number-button">{{ $page }}</a>
                @endif
            @endfor
            ...
            @if ($end < $paginator->lastPage())
                @if ($end < $paginator->lastPage() - 1)
                    <span class="pagination-dots">...</span>
                @endif
                <a href="{{ route('home.page', $paginator->lastPage()) }}" class="pagination-number-button">{{ $paginator->lastPage() }}</a>
            @endif
        </div>

        {{-- Bouton Suivant --}}
        @if ($paginator->hasMorePages())
            <a href="{{ route('home.page', $paginator->currentPage() + 1) }}" class="pagination-button" rel="next">
                <svg class="pagination-icon" xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
